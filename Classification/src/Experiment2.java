...
public class Experiment2 {
	...
	PrintWriter writer = new PrintWriter("experimentalResults/errorEstimateAdaBoostLargeRoundsBoosting.txt", "UTF-8");
	for (hypothesesCount = 10; hypothesesCount < 1001; hypothesesCount++){
		Classifier c = new AdaBoostClassifier(nd, hypothesesCount);
		errorEstimateTest = 1- e.getErrorEstimate (c, nd, e.testLables);
		errorEstimateTrain = 1- e.getErrorEstimateTrain (c, nd, e.testLables);
		System.out.print ("Error Estimate Test:" +  errorEstimateTest + ",");
		System.out.println ("Error Estimate Train:" +  errorEstimateTrain);
		writer.println(hypothesesCount +  "," + errorEstimateTrain + "," + errorEstimateTest);
	}
	writer.close();
    }
