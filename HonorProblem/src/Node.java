 ...
 * generated.  Thus, the path from the initial node to this node can
 * be computed by tracing backward using the parent links.  (The
 * parent of the initial node is set to <tt>null</tt>.)  The total
 * distance from the initial node is equal to the depth of this node.
 */
public class Node {
    ...
    private int depth;
    private Node parent;
    private int cost; // This is the cost of arriving to the current node
	...
	for (int i = 0; i < new_states.length; i++)
	    new_nodes[i] = new Node(new_states[i], depth+1, this, new_states[i].getCost());
