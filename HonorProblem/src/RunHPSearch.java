...
public class RunHPSearch {
	...
	for(int i = 0; i < num_puzzles; i++) {
		...
		soln_cost = new int[num_puzzles][num_heuristics]; 
		...
		if (search.path == null) {
		    ...
		    soln_cost[i][h] = search.getPathCost();
	...
	for (int h = 0; h < num_heuristics; h++)
	    System.out.print(" |    nodes dpth  br.fac cost");
	System.out.println();
		...
		if (soln_depth[i][h] < 0) {
		    ...
		    System.out.print(" | "
				     ...
				     + left_pad(brfac_nf.format(BranchingFactor.compute(num_generated[i][h],
											soln_depth[i][h])),7)
		    		 + " "
		    		 + left_pad(Integer.toString(soln_cost[i][h]), 4));
		}
	    }
	    System.out.println();
	}
